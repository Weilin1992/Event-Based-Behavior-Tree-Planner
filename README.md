...
##1.行为树和状态机
一提到游戏AI，最先想到的应该就是行为树和状态机吧，虽然没有统计过，但个人估计，90%以上的游戏AI，都是用这两种方式实现的，早期的游戏，绝大多数都是使用状态机，但是，由于游戏AI逻辑变得越来越复杂，状态机的编写和维护也变得越来越难。所以，行为树变得越来越流行。
...
看到上面的描述，是不是觉得很简单？爱吃辣条的小明也是这么想的哈哈（结果全是坑，当然，也可能是我菜）接下来就是这个planner的具体实现方案，也可以当做文档阅读。
...
| Name          | Discription                                  |
| ------------- |:-------------------------------------        |
| Start(virtual)|在worldmanager中注册改smart object,注册blackboard，继承类override中要调用该基类函数|
|fixupdate(virtual)|更新blackboard，继承类中override要调用该基类函数|
|上面两个delegate装什么普通成员函数！||
...
| Name          | Discription                                  |
| ------------- |:-------------------------------------        |
|check_precon(virtual)|检查是否满足precondtion|
|check_postcon(virtual)|行为树返回success后设置post condition|
|set_ postcon _database(virtual)|在database中设置post condition，用于事件规划预计算
|restore_database(virtual)|恢复database到precondition状态，用于事件规划计算|
...
| Name          | Discription                                  |
| ------------- |:-------------------------------------        |
|Excute|执行行为树，当行为树返回Success，设置post condition|
...
| Name          | Discription                                  |
| ------------- |:-------------------------------------        |
|getInstance|单例模式，返回该类实例|
|init_BT|根据planner构建行为树|
|Tick|执行行为树|
...
|dfs|dfs事件规划(目测gc爆炸)|
|bfs|bfs事件规划（待完成）|
|pop|partial order planner(待完成)
|huristic_plan|启发式搜索|
|greedy_plan|贪心搜索|
|calculate_transition|使用上面的搜索方法生成转移矩阵|
|reschedule|行为树返回false，重新进行规划|
|h_cost|启发式搜索距离终点距离|
